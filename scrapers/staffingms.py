"""StaffingMS Inhuurdesk - Nétive VMS op Salesforce.

Zelfde onderliggende platform als stedin_vms.py / portofrotterdam.py
(Nétive VMS, Salesforce Experience Cloud): login via
input[name=username/password], daarna een directe Lightning-lijstweergave
op jRequest__c, generiek uitgelezen op kolomkoppen + celwaarden.

Vereist secrets STAFFINGMS_EMAIL en STAFFINGMS_WACHTWOORD.
...
pad blijkt te hebben, meld dat dan zodat AANVRAGEN aangepast kan worden.
"""
...
BASIS = "https://staffingdesk.my.site.com"
...
def _norm(rij):
    ...
    return {
        ...
        "titel": titel,
        "organisatie": pak("Klant") or "StaffingMS",
...
def haal_op():
    ...
    email = os.environ.get("STAFFINGMS_EMAIL")
    wachtwoord = os.environ.get("STAFFINGMS_WACHTWOORD")
    if not email or not wachtwoord:
        print("  STAFFINGMS_EMAIL/WACHTWOORD ontbreekt; overslaan")
        return []
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if not ingelogd:
                try:
                    page.screenshot(path="debug_staffingms_login.png", full_page=True)
                except Exception:
                    pass
                print("  inloggen niet gelukt; bron overgeslagen (geen data), "
                      "zie debug_staffingms_login.png")
                return []
